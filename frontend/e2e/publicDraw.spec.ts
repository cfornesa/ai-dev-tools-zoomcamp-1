...
test.describe('C2.js Interactive visitor drawing (#670)', () => {
  ...
  test('draws temporarily on regular and immersive views and captures the marks', async ({
    ...
    for (const [index, surface] of ['pieces', 'immersive'].entries()) {
      const page = await (await browser.newContext()).newPage();
      await page.setViewportSize(
        index === 0 ? { width: 1280, height: 900 } : { width: 375, height: 812 },
      );
      const writes: string[] = [];
      ...
      await page.screenshot({
        path: `test-results/public-draw-${surface}-marked.png`,
        fullPage: true,
      });
      ...
      await page.screenshot({ path: `test-results/public-draw-${surface}.png`, fullPage: true });
