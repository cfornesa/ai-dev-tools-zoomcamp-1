...
/**
 * Issue #165 (task 133): integration coverage for `LayersPanel.tsx`'s
 * selection-driven `scrollIntoView` effect now that it's gated by
 * `isRowFullyVisible` (see that file's "Issue #165" comment and
 * `LayersPanel.autoScroll.test.ts` for the pure-helper unit tests) rather
 * than calling unconditionally on every selection change.
 *
 ...
 * on screen" vs. "off screen" without needing real layout.
 */
...
describe('LayersPanel auto-scroll (issue #165)', () => {
  ...
  it('calls scrollIntoView when the newly-selected row is out of view', async () => {
    await loadReadyWorkspace();
    const user = userEvent.setup();
    await user.click(screen.getByRole('button', { name: 'Add circle' }));
    await user.click(screen.getByRole('button', { name: 'Add rectangle' }));
    ...
    const circleButton = within(outlineList()).getByRole('button', { name: 'Circle 1' });
    const circleRow = circleButton.closest('li')!;
    // Below the stubbed 800px viewport.
    vi.spyOn(circleRow, 'getBoundingClientRect').mockReturnValue({
      top: 900,
      bottom: 940,
      left: 0,
