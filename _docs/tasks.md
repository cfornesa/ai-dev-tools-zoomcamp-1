...
- #93 Scene canvas has no visible shape rendering: shapes, selection, and handles are all unstyled/invisible — Status: PROPOSED. Found while auditing #90's button-consistency fix: `.editor-scene-shape` and every `.editor-shape-handle` variant (move/resize/rotate/group/vertex) have zero CSS, so the editor's Preview canvas renders no visible shapes, no selection highlight, and no visible handles, even though the underlying hit-testing/drag/resize/rotate/undo-redo logic all works. Also captures: the Tools/Preview/Inspector split into mutually-exclusive tabs below the 1024px `NARROW_BREAKPOINT_PX` must be reworked so live Preview stays reachable alongside Tools/Inspector at every viewport width (explicit hard requirement, not just a desktop nicety); verifying gesture/demo-signal input visibly drives shape movement once rendering exists; and adding a base style for native `input`/`select`/`textarea` controls mirroring the button fix. Substantial enough to likely need sub-tasking rather than one PR — see the issue for full scope.
