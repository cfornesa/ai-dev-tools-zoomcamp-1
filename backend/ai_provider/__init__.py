...
Task 45 (`docs/plan.md`'s "AI provider and cost control" section):
Django exposes exactly two structured AI operations — create-scene and
edit-scene (`docs/plan.md`'s "AI actions") — through a single abstract
...
- `logging.py` — the minimal-metadata operation log, prompt retention
  off by default per `docs/plan.md`: "Log minimal necessary metadata;
  avoid retaining prompts by default unless the product later adds an
  explicit user-facing history choice."

See `docs/plan.md`'s "AI-assisted workflow" and "API-key security"
