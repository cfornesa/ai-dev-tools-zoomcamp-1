...
- **Published boundary:** The repository is synchronized at `origin/main`, but the Replit Git surface reports `Unsupported state: you are in the middle of a rebase` after showing the remote as current. The subsequent Replit publish therefore served the prior frontend bundle; the published gallery still reports all six reference pieces with `thumbnail_is_fallback: true`. No force-reset, direct production SQL, or owner refresh was used. This is a deployment-sync blocker, not a code-test failure.
