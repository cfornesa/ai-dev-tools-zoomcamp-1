...
def test_create_project_with_defaults(user):
    ...
    # Task 51: `docs/plan.md`'s "Remix setting" — "Public projects have
...
def test_postgres_trigger_blocks_raw_sql_snapshot_mutation(django_db_blocker):
    ...
    with django_db_blocker.unblock():
        ...
        with conn.cursor() as cursor:
            ...
            # raw-SQL version insert needs a real `scenes_scene` row to
            # point at first -- mirrors what `BlankProjectCreateView`/every
            # other creation path does at the ORM level.
            cursor.execute(
            ...
            cursor.execute(
                ...
                [project_id, scene_id, json.dumps(BLANK_SCENE)],
            )
