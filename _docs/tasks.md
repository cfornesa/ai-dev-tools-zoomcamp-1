...
- **Next action:** Implement the real-inference diagnostic described above, evaluate whether `delegate: 'CPU'` should become the default or an automatic fallback, remeasure against #192's original budget, and record baseline-vs-post evidence in the issue before any future closure.
