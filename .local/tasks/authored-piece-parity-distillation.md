...
| [#327](https://github.com/cfornesa/ai-dev-tools-zoomcamp-1/issues/327) | Verify manual 3D editor `/projects3d/:id` | Child of #320; one route/surface | `blocked_environment` | Focused manual 3D suite passes 52/52; required disposable-stack Chromium gate is blocked by Docker daemon permission failure. Keep open; exact deployed route verification remains pending |
| [#328](https://github.com/cfornesa/ai-dev-tools-zoomcamp-1/issues/328) | Verify AI 3D editor `/ai-projects3d/:id` | Child of #320; one route/surface | `blocked_environment` | Focused AI 3D suite passes 54/54; required disposable-stack Chromium gate is blocked by Docker daemon permission failure. Keep open; exact deployed route verification remains pending |
...
| [#331](https://github.com/cfornesa/ai-dev-tools-zoomcamp-1/issues/331) | Verify embedded 2D viewer `/embed/p/:id` | Child of #320; one route/surface | `handed_off_verification_boundary` | Exact `embed2dStageChrome.spec.ts` passes 1/1 locally for the chrome-less route and stage-local controls. GitHub `QA: FAIL` comment `5510417053` records the boundary; published revision remains unverified |
...
| [#338](https://github.com/cfornesa/ai-dev-tools-zoomcamp-1/issues/338) | Manual 2D stage-local publication parity `/projects/:id` | Child of #320; local implementation slice | `closed_completed_local` | GitHub closed completed after focused 67/67, full frontend 189/2,386, and exact Docker-backed Chromium route 1/1. This is local implementation closure only; exact deployed verification remains #325 |
| [#339](https://github.com/cfornesa/ai-dev-tools-zoomcamp-1/issues/339) | AI 3D stage-local publication parity `/ai-projects3d/:id` | Child of #320; local implementation slice | `closed_completed_local` | GitHub closed completed after focused 54/54, full frontend 189/2,386, and exact Docker-backed Chromium route 1/1. This is local implementation closure only; exact deployed verification remains #328 |
| [#340](https://github.com/cfornesa/ai-dev-tools-zoomcamp-1/issues/340) | AI 2D stage-local publication parity `/ai-projects/:id` | Child of #320; local implementation slice | `closed_completed_local` | GitHub closed completed after focused 34/34, full frontend 189/2,386, and exact Docker-backed Chromium publication route 1/1. This is local implementation closure only; exact deployed verification remains #326 |
| [#341](https://github.com/cfornesa/ai-dev-tools-zoomcamp-1/issues/341) | Manual 3D stage-local publication parity `/projects3d/:id` | Child of #320; local implementation slice | `closed_completed_local` | GitHub closed completed after focused 52/52, full frontend 189/2,386, and exact Docker-backed Chromium route 1/1. This is local implementation closure only; exact deployed verification remains #327 |
...
| [#343](https://github.com/cfornesa/ai-dev-tools-zoomcamp-1/issues/343) | Immersive 3D touch d-pad travel | Immersive capability; route variants decide inclusion in #333–#335 | `completed` | `26aed1f` adds bounded held zoom in/out alongside the four directional controls. Focused tests pass 7/7; full frontend tests pass 2,385/2,385; typecheck/build/lint pass; exact browser transaction passes 4/4. #334/#335 explicitly record that custom/CMS embeds include the shared d-pad; GitHub issue closed after QA PASS |
| [#344](https://github.com/cfornesa/ai-dev-tools-zoomcamp-1/issues/344) | Immersive 3D hand gesture move/strafe | Depends on #295; route evidence remains #333–#335 | `handed_off_verification_boundary` | Commit `7ea23da` ships bounded held-pinch forward/back/strafe, safe-stop signal resets, guide copy, focused tests, and Docker browser guide evidence. Keep open because the required Chromium/manual-camera movement evidence and exact deployed route evidence are not available |
| [#345](https://github.com/cfornesa/ai-dev-tools-zoomcamp-1/issues/345) | 3D per-voice instrument selectors | Depends on #306/#310; route/artifact evidence remains #327–#337 | `completed` | GitHub `QA: PASS` comment `5510368100` posted and issue closed completed after 49/49 focused tests, 1/1 deterministic Chromium smoke, full frontend/static gates, and selector-only closure evidence |
...
## End-of-blocked-issue distillation: #327 (2026-09-02)

Status: DISTILLATION COMPLETE — #327 LOCALLY PARTIAL, ENVIRONMENT-BLOCKED; #328
REMAINS THE NEXT INDEPENDENT TRANSACTION

- Duplicate check: no new issue is warranted. #327 owns only the manual 3D
  owner route; #341 owns local manual 3D implementation, #320 owns release
  reconciliation, and sibling route/artifact issues retain their boundaries.
- Blocker check: the focused 3D suite passed 52/52, but
  `BROWSER_QA_E2E_SPEC=e2e/manual3dStageChrome.spec.ts make browser-qa` could
  not start because Docker cannot access its server socket. The issue body
  records the exact command and `docker info` permission failure.
- Closure check: #327 remains open. The real-browser control matrix and exact
  deployed revision are not proven; local component evidence is insufficient.
- Atomicity check: no split is needed. This is one finite authenticated manual
  3D route verification boundary.
- Progress rule: #328 is independent and may proceed; retry #327 only after
  Docker access and the reviewed deployment revision are available.

## End-of-blocked-issue distillation: #328 (2026-09-02)

Status: DISTILLATION COMPLETE — #328 LOCALLY PARTIAL, ENVIRONMENT-BLOCKED; next
independent public-surface transaction remains eligible

- Duplicate check: no new issue is warranted. #328 owns only the AI 3D owner
  route; #339 owns local AI 3D implementation, #320 owns release reconciliation,
  and sibling route/artifact issues retain their boundaries.
- Blocker check: the focused AI 3D suite passed 54/54, but
  `BROWSER_QA_E2E_SPEC=e2e/ai3dStageChrome.spec.ts make browser-qa` could not
  start because Docker cannot access its server socket. The issue body records
  the exact command and `docker info` permission failure.
- Closure check: #328 remains open. The real-browser control matrix and exact
  deployed revision are not proven; local component evidence is insufficient.
