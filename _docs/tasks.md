...
Evidence (2026-08-27): Implemented in commit `4f27dc7` with explicit direct-layer mode, owning-layer synchronization for shape/group selection, complete layer-block markers, and layer-control hit testing. Focused frontend tests pass (66); full frontend tests pass (1,874 across 127 files); frontend build/typecheck/lint/format and `make check` pass (backend 629 passed/22 skipped). QA comment [#5433947388](https://github.com/cfornesa/ai-dev-tools-zoomcamp-1/issues/183#issuecomment-5433947388) records FAIL only because the required real-browser desktop/narrow regressions were not run in this session.
...
Next action: Start the approved PostgreSQL + Django (`AI_PROVIDER=fake`) + Vite stack and execute the #183 desktop and narrow Playwright regressions; update the QA comment and close only after that browser gate passes.
