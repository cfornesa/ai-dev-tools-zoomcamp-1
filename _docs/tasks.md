...
- #95 Editor workspace isn't mobile-friendly: header layout, icon affordances, accordion defaults, demo-controls alignment — Status: PROPOSED. Filed after a live review of #94's unified editor at mobile/tablet widths: the shell header (`Layout.tsx`) and editor header (`EditorWorkspace.tsx`'s `.editor-workspace-header`) don't reflow for narrow viewports (heading/hamburger should split left/right; the editor header should stack into title+edit-icon / saved-status / visibility / publish+exit-icon rows on mobile and tablet, collapsing status/visibility to a second row only at >=1024px); "Edit title" and "Exit without saving" should become pencil/X icon affordances; the Preview canvas overflows its panel at some widths; `DemoControlsPanel.tsx`'s continuous-signal sliders aren't column-aligned; and (per a follow-up review comment) every `CollapsibleSection.tsx` accordion section should default to closed (not open, as #94 left them), "Camera & demo controls" should split into independent "Camera" and "Demo signal controls" sections, and Demo signal controls needs "Controls"/"Sensitivity"/"Gesture details" subheadings. See the issue for full acceptance criteria and attached screenshots.
