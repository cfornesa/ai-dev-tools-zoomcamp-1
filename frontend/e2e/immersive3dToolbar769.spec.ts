...
/**
 * Issue #769: the immersive view of a structured 3D piece (canonical
 * `/users/@handle/immersive/:slug`) shows the icon row from docs/piece-toolbar-parity-matrix.md
 * in order (Screenshot, Download, Sound, Piece controls, Hand gesture guide, Fullscreen last; no
 * self-link to the immersive view) and Steer lives inside the Piece controls popover.
 */
const SCENE = {
  schemaVersion: 1,
...
test.describe('structured 3D immersive view toolbar (#769)', () => {
  test('exact icon row and Steer inside Piece controls', async ({ page }, testInfo) => {
    ...
    for (const viewport of [
      ...
      await page.goto('/users/@artist/immersive/red-sphere');
      ...
      await page.screenshot({ path: testInfo.outputPath(`immersive-3d-${viewport.width}.png`) });
    }
  });
});
