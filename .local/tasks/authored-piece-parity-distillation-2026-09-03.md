...
## #380 and #379 completion reconciliation — 2026-09-03

Replit synchronization was repaired through the authenticated Git panel,
which reconciled `main` to `origin/main` at `cfd16d1` and intentionally
republished the reviewed revision. The custom domain now serves
`index-xwmMEBBo.js`, byte-identical to the fresh local build. Exact editor
inspection confirmed the stage-local controls and Draft/Published panel;
Published was exercised and the fixture was restored to Draft. Fresh
anonymous public inspection confirmed the stage-local hamburger and permitted
Screenshot, Download, Piece controls, and Fullscreen overlay. Fixed-viewport
QA for the same build covered 1280x900 and 375x812. #380 and #379 are
permanently closed as `completed`; no closed issue was reopened. Later gaps
require new linked criterion-ready issues.
...
The authorized Chrome session reached the exact owner route and activated
`Steer the piece`; the rendered status confirmed active local camera tracking.
The remaining criteria require owner-performed held pinch, release, hand-loss,
disable/stop, and denial/unavailable checks that automation cannot synthesize.
QA comment 5533168795 records the terminal blocker. End-of-blocker task
...
The exact authenticated owner route and exact public 2D route were reopened
in the current signed-in Chrome session. Both intentionally show only the
...
switch. These current observations do not reproduce the reported complete
absence of controls or publication transition.
...
Draft and the anonymous embed returned its unavailable/private state. GitHub
