...
8. Run a routing audit against `.agents/skills/_shared/HANDOFF-CONTRACT.md`: for every processed issue, confirm the ledger records the actual stage owner (`service / model / effort`) for all five stages — scoping, implementation, optional second-opinion review, QA, and the readiness gate — with every Claude substitution for a rostered service flagged, and confirm no second-opinion stage was credited to the model that wrote the diff. Report stages whose owner was not recorded as an unresolved reconciliation gap; do not backfill provenance by inference. Confirm the readiness gate ran on Opus 5 — the one stage that never routes to another service or a lesser model, at the owner's budgeted effort.
...
- the routing audit: per-issue stage owners, flagged substitutions, unrecorded-provenance gaps, and the model/effort the readiness gate actually ran on.
