...
- **Status:** ACTIVE (reopened 2026-08-28)
...
- **Evidence (2026-08-28, GPU delegate fallback):** Commit `47ec6b2` implements the concrete candidate root cause: `createFromOptions` now retries once with `delegate: 'CPU'` if `delegate: 'GPU'` creation throws, instead of surfacing an unrecoverable error; `getDiagnostics().activeDelegate` (`'GPU' | 'CPU' | null`) makes the active delegate observable. 3 new deterministic tests. `make check` full pass (backend 665/22 skipped, frontend 1,922/131 files). Real Chromium, isolated PostgreSQL-backed stack: `BROWSER_QA_E2E_SPEC=e2e/publishingAndRemix.spec.ts make browser-qa` 24/24 passed, no regression, camera diagnostics still well within budget (0 long tasks at both viewports this run). QA comment posted on #192. **This addresses GPU delegate creation failing outright; it does not address a GPU delegate that creates successfully but performs slowly at inference time** — that still needs the real non-seam profiling harness below. Issue kept open; not closed on this evidence alone, per [[camera-synthetic-verification-gap]].
- **Next action:** Implement the real-inference diagnostic described above, evaluate whether `delegate: 'CPU'` should become the default (vs. today's fallback-only-on-creation-failure), remeasure against #192's original budget, and get real-camera or production-path (`animate.creatrweb.com`) confirmation before any future closure. No physical camera or Replit deployment access is available to this agent — that remains a standing verification boundary, not a task to silently skip.
