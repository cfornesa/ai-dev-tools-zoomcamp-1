 ...
 *
 * Consolidation note: this file's 5 originally independent test()s are
 * grouped into 2 using test.step() per original scenario, each opening
 * its own fresh browser.newContext()/page. `fallbackReference` is set
 * once by the first step and read by every later one -- an ordering
 * dependency this file already had before consolidation (declaration
...
test.describe('Generated thumbnail service: capture artwork instead of hash-derived placeholders (#438)', () => {
  ...
  test('creating a piece through the real Studio, and revising it from the Editor, each replace the fallback with a real capture', async ({
    browser,
  }) => {
    test.setTimeout(60000);

    await test.step('creating a piece through the real Studio replaces the fallback with a real, correctly-sized capture', async () => {
    ...
    await test.step('a revision saved from the Editor captures a fresh thumbnail for the new version, leaving the original version untouched', async () => {
      const context = await browser.newContext();
      const page = await context.newPage();
      await loginViaUI(page, fixture.owner.email, fixture.password);
  ...
  test('the Regenerate thumbnail button captures off-screen, a failed capture leaves the fallback until retried, and cross-user upload/public-route access stay denied', async ({
    browser,
  }) => {
    test.setTimeout(60000);

    await test.step("the editor's Regenerate thumbnail button captures the current version off-screen", async () => {
      const context = await browser.newContext();
      const page = await context.newPage();
      await loginViaUI(page, fixture.owner.email, fixture.password);
      const created = await apiPost(context, '/api/art-pieces/', {
    ...
    await test.step('a failed capture leaves the fallback in place, and retrying replaces it', async () => {
      const context = await browser.newContext();
      const page = await context.newPage();
      await loginViaUI(page, fixture.owner.email, fixture.password);
      const created = await apiPost(context, '/api/art-pieces/', {
    ...
    await test.step("a non-owner cannot upload a thumbnail, and an unpublished piece's thumbnail stays unreachable through the public route even after a real capture", async () => {
      const context = await browser.newContext();
      const page = await context.newPage();
      await loginViaUI(page, fixture.owner.email, fixture.password);
      const created = await apiPost(context, '/api/art-pieces/', {
