...
`backend/backend/settings.py` reads several required environment variables (see
...
always takes priority) and then defers to `backend.settings` for
everything else, so `uv run pytest` works offline on a clean checkout
without requiring a `.env` file.

`DATABASE_URL` is given a placeholder value purely so `backend.settings`
...
tested directly against `backend.settings` in `tests/test_env_config.py`.
"""
