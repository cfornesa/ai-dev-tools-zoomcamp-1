...
describe('p5 scene preview', () => {
  ...
  it('recomposes the live camera at its new artwork-relative layer after a layer-order change', () => {
    ...
    // The top artwork layer is above the camera at its initial position.
    expect(pixel(preview.getCanvasElement()!, 20, 20)).toEqual([0, 0, 255, 255]);

    preview.render(scene, [], [], false, { ...overlay, layerOrder: 3 });
    // The camera now paints above the top artwork layer, proving that a
    // subsequent live-editor render applies the changed Layers position to
    // the same compositor rather than leaving the camera at its old order.
