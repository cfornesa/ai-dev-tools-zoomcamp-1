...
- **Latest publish verification:** Replit `creatrweb` pulled the safely pushed `2b91e26` correction through its guarded workflow, synchronized at Replit commit `76bce2c`, and promoted the current release. The renderer correction `019533a` is locally verified and awaits the next owner-authorized Replit pull/republish. The authenticated Chrome owner tab previously verified the refresh controls and invoked the affected sphere refresh. `PUBLISHED_APP_URL=https://augmentrart.com scripts/smoke-published.sh` passed `/health/` but remains non-zero because `/__share-metadata-status` reports `backend_reachable:false` with sanitized `TypeError: fetch failed`.
