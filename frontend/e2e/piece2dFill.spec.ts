...
const fixtures = {
  canvas2d: '<canvas id="piece-canvas" width="320" height="240"></canvas><script>const c=document.querySelector("canvas"); c.getContext("2d").fillRect(0,0,320,240);</script>',
  ...
  'c2js-interactive': 'window.sketch = ({ canvas, startFrame }) => { canvas.addEventListener("pointermove", (event) => { canvas.dataset.pointer = `${event.offsetX},${event.offsetY}`; }); const x = canvas.getContext("2d"); startFrame(() => { x.fillStyle = "#172554"; x.fillRect(0, 0, canvas.width, canvas.height); }); };',
} as const;
...
test.describe('Generated 2D regular stage fill (#705)', () => {
  ...
  test('fills, resizes, preserves pointer coordinates, and supplies inspected screenshots for every 2D engine', async ({
    ...
    const profile = (await (await apiGet(context, '/api/account/profile/')).json()) as { handle: string };
    ...
    for (const engine of Object.keys(fixtures) as Array<keyof typeof fixtures>) {
      ...
      expect((await apiPatch(context, `/api/art-pieces/${piece.public_id}/`, { status: 'published' })).status()).toBe(200);
    ...
    for (const viewport of [{ width: 1440, height: 900 }, { width: 375, height: 812 }]) {
      ...
      for (const engine of Object.keys(fixtures) as Array<keyof typeof fixtures>) {
        ...
        const surface = frame.locator(engine === 'svg' ? 'svg' : engine === 'p5js' ? 'canvas' : engine === 'canvas2d' ? '#piece-canvas' : '#c2-canvas');
        await expect(surface).toBeVisible({ timeout: 15_000 });
        const geometry = await surface.evaluate((element) => {
          const rect = element.getBoundingClientRect();
          const stage = element.ownerDocument.body.getBoundingClientRect();
          return { surfaceWidth: rect.width, stageWidth: stage.width, surfaceHeight: rect.height, stageHeight: stage.height };
        });
        ...
        await page.screenshot({ path: testInfo.outputPath(`issue-705-${engine}-${viewport.width}.png`), fullPage: true });
      }
    }
  });
});
