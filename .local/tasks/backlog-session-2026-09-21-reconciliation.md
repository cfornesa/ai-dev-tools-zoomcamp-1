...
- **#719 evidence:** the authenticated production `/api/projects3d/` payload confirmed the affected `Untitled 3D scene` has a sphere in `scene_json`, but its existing thumbnail was treated as a successful render (`thumbnail_is_fallback: false`) despite being stale for the owner's expected presentation. The renderer's documented sphere approximation is a camera-facing ellipse, so a refreshed image can remain circular; this is renderer fidelity, not a failed owner refresh.
- **#719 engineering/QA:** owner-only `POST /api/projects3d/<id>/thumbnail/refresh/` now explicitly re-renders the current version even when an existing thumbnail is non-fallback; the studio card exposes `Refresh thumbnail` for every current 3D version and retains `Retry thumbnail` wording for fallback states. Commit `151e61d` added the owner path; correction commit `2b91e26` extended it for stale successful renders. Focused backend tests passed `13`; focused Project3DCard tests passed `14`; frontend typecheck, backend Ruff/format, and diff checks passed. Full `make check` earlier had one unrelated flaky `App.embedRoute.test.tsx` failure that passed on focused rerun.
- **#719 published owner verification:** through the authenticated Chrome owner session, Replit pulled the correction and promoted a release just now. The live `/studio` route shows `Refresh thumbnail` on all three 3D cards; the affected public sphere refresh was invoked. An uncached thumbnail request returned a freshly rendered image, still circular per the documented billboard sphere approximation. No direct production SQL or unauthorized refresh was used.
...
- **Latest publish verification:** Replit `creatrweb` pulled the safely pushed `2b91e26` correction through its guarded workflow, synchronized at Replit commit `76bce2c`, and promoted the current release. The authenticated Chrome owner tab verified the new 3D refresh controls and invoked the affected sphere refresh. `PUBLISHED_APP_URL=https://augmentrart.com scripts/smoke-published.sh` passed `/health/` but remains non-zero because `/__share-metadata-status` reports `backend_reachable:false` with sanitized `TypeError: fetch failed`.
