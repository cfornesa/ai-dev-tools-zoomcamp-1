...
| [#321](https://github.com/cfornesa/ai-dev-tools-zoomcamp-1/issues/321) | Docker browser verification must select and fingerprint the correct project | Independent workflow item; repository Compose definition now exists | `dependency-blocked` | `workflow/infrastructure-defect`; preflight correctly identifies the running sibling project, but two repository Compose build attempts hit Docker registry `DeadlineExceeded` for uncached base images; retry when registry metadata is reachable, then close only with correct-stack evidence |
