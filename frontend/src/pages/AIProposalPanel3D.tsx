...
function AIProposalPanel3D({
  ...
  // Issue #463: the persisted "Agent workflow" run, offered alongside the
  // one-shot flow above rather than replacing it -- mirrors
  // `AIProposalPanel.tsx`'s identical #462 wiring exactly, reusing the
  // same `useAIRun` orchestrator (`target_type: 'project3d'`) and the
  // same shared `AIRunPanel` component, not a second implementation of
  // either. `Project3D.current_version` is already the full nested
  // `SceneVersion3D` object (unlike 2D, which needs a separate
  // `getSceneVersion` call) -- see `api/projects3d.ts`'s own doc comment
  // on that field -- so `fetchAcceptedVersion` just refetches the project
  // and reads it back.
  const fetchAccepted3DVersion = useCallback(async (pid: string): Promise<SceneVersion3D> => {
    const project = await getProject3D(pid);
    if (!project.current_version) {
      throw new Error('The accepted version could not be found.');
    }
  ...
  // Issue #463's own acceptance criterion mirrors #462's: resolve
  // selection to stable ids. Only `scene.objects` are offered (matching
  // the issue's "select cube only" fixture) -- lights and the camera are
  // never individually selectable for a scoped edit in this MVP.
  const aiRunSelectableObjects: AIRunSelectableObject[] = workingCopy
    ? workingCopy.objects.map((object) => ({
        id: object.id,
        label: object3DLabel(object, workingCopy.objects),
      }))
    : [];
  ...
  return (
    <div className="ai-proposal-panel">
      ...
      {/* Issue #463: mirrors `AIProposalPanel.tsx`'s identical #462 toggle. */}
      ...
      {workflowMode === 'agent' && (
        <AIRunPanel
          ...
          editSelectionLabel="Edit selected object"
          noSelectableObjectsMessage="No objects in this scene yet."
          renderCandidatePreview={(scene) => (
            <Scene3DPreview
              scene={scene as unknown as Scene3DDocument}
          ...
          <form aria-label={MODE_LABELS[mode]} onSubmit={handleSubmit}>
            ...
            {pending && (
              <button
                ...
                data-testid="ai-3d-cancel-generation"
              >
                Cancel
              </button>
            )}
          ...
          {phase === 'success' && proposal && (
            <section aria-label="AI proposal preview" data-testid="ai-3d-proposal-success">
              ...
              <p data-testid="ai-3d-proposal-scene-summary">
                {proposedScene?.objects?.length ?? 0} object(s),{' '}
                {proposedScene?.lights?.length ?? 0} light(s), {proposedScene?.groups?.length ?? 0}{' '}
                group(s) proposed.
              </p>
