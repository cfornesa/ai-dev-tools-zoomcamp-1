...
GitHub is authoritative for state. Sixteen issues were open after this pass;
the six new records are #485–#490.
...
| 3 | [#445](https://github.com/cfornesa/ai-dev-tools-zoomcamp-1/issues/445) | Reconcile one exact release candidate | Every required child and exact release identity | DEPENDENCY-BLOCKED parent container | Release QA only; never select for implementation. Add #485–#490 to the current child evidence set. |
...
| 16 | [#486](https://github.com/cfornesa/ai-dev-tools-zoomcamp-1/issues/486) | Choose generated-gallery discoverability architecture | Repository owner decision | BLOCKED / product-design decision | Owner selects Unified, Sibling, Gateway, Reframe, or another direction; re-distill one unconditional stage 2a/2b implementation issue. |

## New criterion-ready issue definitions

The complete self-contained contracts, fixtures, commands, out-of-scope
boundaries, and routing hints are authoritative in the six GitHub bodies:
...
#490 have unconditional stage 2a routing under current evidence. #486 is a
decision issue precisely because its implementation route would otherwise be
conditional; it must produce one new unconditional implementation issue after
the owner chooses.
...
- Closed #400 already owns and passed rendered QA for the branded responsive
  ...
  mismatch.
- Closed #392 owns the structured 2D/3D contents of `/gallery`; #486 does not
  reopen it or merge generated `ArtPiece` data silently. It owns only the new
...
| Generated public gallery has no anonymous navigation path | implementation gap plus owner design decision | New #486 decision issue | Owner chooses one presented direction; task-distillation then creates one implementation child. |
...
| Generated gallery contains no public cards | verification-boundary for card visuals, not a defect | #486 decision context | Use a deterministic public generated-piece fixture in the eventual implementation issue. |
...
artifact QA. #486 stays blocked until the owner selects a public information
architecture. #489 and #490 are independent but require exact published proof.
...
Existing deployment and parity topics remain applicable. Two durable updates
are proposed, not silently written, per repository ownership rules:
