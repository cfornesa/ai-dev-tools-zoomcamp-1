...
| [#329](https://github.com/cfornesa/ai-dev-tools-zoomcamp-1/issues/329) | Verify public 2D viewer `/p/:id` | Child of #320; one route/surface | `open` | Live FAIL: supplied URL has legacy sibling controls/no stage toolbar; local source also currently hides the active public camera video. Fix/verify this route after publish |
...
| Public 2D viewer / embed | Deployed route still serves old sibling-panel shell; local public 2D camera video is explicitly `visibility: hidden`, so active camera composition is not proven and likely fails the reference contract | `implementation-defect + verification-boundary` | #329/#331; engineer #329 route behavior, then publish and verify exact public/embed routes |
