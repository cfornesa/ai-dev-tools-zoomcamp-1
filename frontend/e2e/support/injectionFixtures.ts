/**
 * Task 74 (issue #74): e2e-local duplicate of
 * frontend/src/export/injectionFixtures.ts, for
 * frontend/e2e/injectionArtifacts.spec.ts (the real-Chromium half of the
 * injection audit). A systematic, fixture-driven hostile-payload catalog
 ...
 *    own.
 */
/** Deliberately not imported from '../../src/api/projects.ts' -- see this
 * directory's own exportHarness.ts doc comment for why importing
 * frontend/src modules into the strict-nodenext e2e TS program is avoided
 * (transitively pulls in extensionless production-source imports that
 * fail nodenext's explicit-extension rule). Duplicated locally instead --
 * this is the *fixture data*, kept in sync by hand with
 * frontend/src/export/injectionFixtures.ts, which is the canonical
 * version used by the jsdom unit suite
 * (frontend/src/export/injectionFixtures.test.ts). */
