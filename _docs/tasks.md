...
- #106 mypy fails on `tests/test_git_safe_push.py`: `BaseServer` has no attribute `repository_root` — Status: PROPOSED. Found while scoping the Replit-authored Git safe-push commits (task #77 / issue #104) for Replit and local deployment readiness: `make check`/CI's "Backend checks" job (`uv run mypy .`) has been failing on `main` since commit b88da12 and remains red on the current tip, confirmed via `gh run list`/`gh run view` (only the mypy step fails; `uv run pytest tests/test_git_safe_push.py` passes 9/9). Root cause: `_AuthenticatedGitHandler._serve_git` (line 86) reads `self.server.repository_root`, but `self.server` is statically typed as `socketserver.BaseServer` by `BaseHTTPRequestHandler`; `repository_root` is only ever assigned dynamically on a locally-defined `Server(ThreadingHTTPServer)` subclass inside the `authenticated_git_remote` fixture. [Issue #106](https://github.com/cfornesa/ai-dev-tools-zoomcamp-1/issues/106).
