...
| 5 | Production-readiness gate | Claude (Opus 5 or Sonnet 5, owner-budgeted effort; owner-authorized GPT-5 substitution permitted) | skill `production-readiness` |
...
- Stage 5 is rostered to Claude Opus 5 or Sonnet 5. When neither is available,
  the owner may authorize GPT-5 for the current session as an explicit
  substitution. Record the authorization in `DECISIONS.md` and the actual
  service/model/effort in the ledger; do not present the substitution as a
  rostered Claude run. No other model or service may substitute.
