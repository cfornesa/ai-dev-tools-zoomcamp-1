...
test.describe('Layers panel', () => {
  ...
  test('renders as its own dedicated landmark, stays collapsible, and keeps every row control inside its bounded width', async ({
    browser,
  }: {
    browser: Browser;
  }) => {
    await test.step('renders as its own dedicated landmark, distinct from Tools', async () => {
    ...
    await test.step('collapses and reopens the mounted Layers panel at desktop and narrow widths', async () => {
      const context = await browser.newContext();
      const page = await context.newPage();
    ...
    await test.step('keeps the panel and every layer-row control inside one bounded width', async () => {
      const context = await browser.newContext();
      const page = await context.newPage();
  ...
  test('selects a layer from its name and Visible control, syncs back from the canvas outline, and stays usable in the narrow layout', async ({
    browser,
  }) => {
    test.setTimeout(60000);

    await test.step('selects a layer from its name and Visible control, then syncs back from the canvas outline', async () => {
      const context = await browser.newContext();
      const page = await context.newPage();
      await loginViaUI(page, fixtures.owner.email, fixtures.password);
      await createBlankProjectViaUI(page);
    ...
    await test.step('keeps bidirectional selection usable in the narrow panel layout', async () => {
      const context = await browser.newContext();
      const page = await context.newPage();
  ...
  test('pointer drag-and-drop, keyboard reorder, and a locked-layer drop rejection all land on the correct scene order', async ({
    browser,
  }) => {
    test.setTimeout(90000);

    await test.step('pointer drag-and-drop and keyboard reorder both land in the same canonical scene order, verified on the canvas and after reload', async () => {
      const context = await browser.newContext();
      const page = await context.newPage();
      await loginViaUI(page, fixtures.owner.email, fixtures.password);
      await createBlankProjectViaUI(page);
    ...
    await test.step('rejects a drop onto a locked layer, leaving the outline unchanged', async () => {
      const context = await browser.newContext();
      const page = await context.newPage();
      await loginViaUI(page, fixtures.owner.email, fixtures.password);
      await createBlankProjectViaUI(page);
